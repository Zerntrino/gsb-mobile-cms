...
export class ListComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.position =
      this.router.routerState.snapshot.root.queryParams['position'] || '';
  ...
  fetch(): void {
    ...
    this.userService.getList(params).subscribe(
      (response) => {
        ...
        this.list = response.data as Admin[];
      },
      (error) => {
        console.log(error);
      }
    );
    this.userService.getListTotal(params).subscribe(
      (response) => {
  ...
  showDetailClick(item: Admin, edit: boolean) {
    console.log(item);
    this.detail = item;
